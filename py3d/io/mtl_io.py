...
from py3d.io.utils import _open_file, _read_image
...
def make_mesh_texture_atlas(
    ...
    atlas = np.ones(size=(F, R, R, 3), dtype=np.float32)
    ...
    for material_name, props in material_properties.items():
        ...
        faces_material_ind = face_material_names == material_name
    ...
    for material_name, image in list(texture_images.items()):
        ...
        image = np.flip(image, [0]).type_as(faces_verts_uvs)
...
def make_material_atlas(
    image: np.ndarray, faces_verts_uvs: np.ndarray, texture_size: int
) -> np.ndarray:
    r"""
    ...
    bary = np.zeros((R, R, 3), device=device)  # (R, R, 3)
    slc = np.arange(2, device=device)[:, None]
    # w0, w1
...
def _bilinear_interpolation_vectorized(
    image: np.ndarray, grid: np.ndarray
) -> np.ndarray:
    """
    ...
    grid = grid * np.array([W - 1, H - 1], dtype= grid.dtype)
    weight_1 = grid - grid.int()
    weight_0 = 1.0 - weight_1

    grid_x, grid_y = grid.unbind(-1)
    y0 = grid_y.astype(np.int64)
    y1 = (grid_y + 1).astype(np.int64)
    x0 = grid_x.astype(np.int64)
    x1 = x0 + 1
...
MaterialProperties = Dict[str, Dict[str, np.ndarray]]
TextureFiles = Dict[str, str]
TextureImages = Dict[str, np.ndarray]


def _parse_mtl(f: str) -> Tuple[MaterialProperties, TextureFiles]:
    material_properties = {}
    texture_files = {}
    material_name = ""
    ...
    for material_name in used_material_names:
        if material_name in texture_files:
            ...
            if os.path.exists(path):
                image = (
                    _read_image(path, format="RGB") / 255.0
                )
...
def load_mtl(
    ...
    *,
    ...
    """
    material_properties, texture_files = _parse_mtl(f)
