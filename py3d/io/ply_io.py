...
class _VertsData:
    ...
    verts: np.ndarray
    verts_colors: Optional[np.ndarray] = None
    verts_normals: Optional[np.ndarray] = None
    verts_texture_uvs: Optional[np.ndarray] = None
...
def _get_verts(header: _PlyHeader, elements: dict) -> _VertsData:
    ...
    if vertex_head.count == 0:
        ...
        return _VertsData(
            verts=verts, verts_colors=np.zeros((0, 3), dtype=np.float32)
        )
    ...
    if (
        ...
        return _VertsData(verts=_make_array(vertex[0], cols=3, dtype=np.float32))
    ...
    if len(vertex) == 1:
        ...
        verts = np.array(vertex[0][:, column_idxs.point_idxs], dtype=np.float32)
        if column_idxs.color_idxs is not None:
            vertex_colors = column_idxs.color_scale * np.array(
                vertex[0][:, column_idxs.color_idxs], dtype=np.float32
            )
        if column_idxs.normal_idxs is not None:
            vertex_normals = np.array(
                vertex[0][:, column_idxs.normal_idxs], dtype=np.float32
            )
        if column_idxs.texture_uv_idxs is not None:
            vertex_texture_uvs = np.array(
                vertex[0][:, column_idxs.texture_uv_idxs], dtype=np.float32
            )
    else:
        ...
        verts = np.empty(size=(vertex_head.count, 3), dtype=np.float32)
        for axis in range(3):
        ...
        if column_idxs.color_idxs is not None:
            vertex_colors = np.empty(
                size=(vertex_head.count, 3), dtype=np.float32
            )
...
class _PlyData:
    ...
    verts_colors: Optional[np.ndarray]
    verts_normals: Optional[np.ndarray]
    verts_texture_uvs: Optional[np.ndarray]


def _load_ply(f) -> _PlyData:
    """
    Load the data from a .ply file.
    ...
    if face is None:
        ...
        faces = np.array(np.vstack(face_arrays), dtype=np.int64)
    else:
        face_list = []
...
def load_ply(f) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def _save_ply(
    ...
    *,
    verts: np.ndarray,
    faces: Optional[npt.NDArray[np.int64]],
    verts_normals: Optional[np.ndarray],
    verts_colors: Optional[np.ndarray],
    ascii: bool,
...
def save_ply(
    f,
    verts: np.ndarray,
    faces: Optional[npt.NDArray[np.int64]] = None,
    verts_normals: Optional[np.ndarray] = None,
    ascii: bool = False,
    decimal_places: Optional[int] = None,
) -> None:
