...
from py3d.io.mtl_io import load_mtl, make_mesh_texture_atlas
from py3d.io.utils import _check_faces_indices, _make_array, _open_file, PathOrStr
...
def _format_faces_indices(faces_indices, max_index: int, pad_value=None):
    """
    ...
    """
    faces_indices = _make_array(
        faces_indices, cols=3, dtype=np.int64
    )
...
def _load_obj(
    ...
    *,
    ...
    verts = _make_array(verts, cols=3, dtype=np.float32)  # (V, 3)
    normals = _make_array(
        normals,
        cols=3,
        dtype=np.float32    
    )  # (N, 3)
    verts_uvs = _make_array(
        verts_uvs,
        cols=2,
        dtype=np.float32,
    )  # (T, 2)
    ...
    if material_colors and not material_names:
        ...
        if isinstance(faces_materials_idx, np.ndarray):
            faces_materials_idx = faces_materials_idx.clip(min=0)
...
def save_obj(
    ...
    *,
    ...
    faces_normals_idx: Optional[np.ndarray] = None,
    verts_uvs: Optional[np.ndarray] = None,
    faces_uvs: Optional[np.ndarray] = None,
...
def _write_normals(
    normals: np.ndarray, faces_normals_idx: np.ndarray, float_str: str
) -> str:
...
def _write_faces(
    f,
    faces: np.ndarray,
    faces_uvs: Optional[np.ndarray],
    faces_normals_idx: Optional[np.ndarray],
) -> None:
    F, P = faces.shape
    for i in range(F):
